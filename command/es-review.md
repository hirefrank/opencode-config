---
description: Perform exhaustive code reviews using swarm-based multi-agent analysis
---

# Review Command (Swarm Edition)

<command_purpose> Perform exhaustive code reviews using parallel worker agents with coordinator synthesis. </command_purpose>
...
**Cost Model**: 1 Opus + 4 Flash = ~70% cheaper than 4x Opus sequential

## Quick Start (Recommended)

Use the shell script for automatic swarm orchestration:
...
If you need fine-grained control, invoke agents directly:

```bash
# Run workers in parallel (4 terminals)
opencode @review-security < worker-security.md
opencode @review-performance < worker-performance.md
opencode @review-cloudflare < worker-cloudflare.md
opencode @review-design < worker-design.md

# Then synthesize with coordinator
opencode @reviewer < coordinator-synthesis.md
```

## Introduction

<role>Senior Code Review Architect coordinating specialized worker agents</role>
...
- OpenCode CLI with swarm worker agents configured
</requirements>
...
9a. Task frontend-design-specialist(PR content) - Identify generic patterns (Inter fonts, purple gradients, minimal animations) - Map aesthetic improvements to Tailwind/shadcn/ui code - Prioritize distinctiveness opportunities - Ensure brand identity vs generic "AI aesthetic"

9b. Task shadcn-ui-architect(PR content) - Validate shadcn/ui component usage and props (via MCP if available) - Check customization depth (`ui` prop usage) - Ensure consistent component patterns - Prevent prop hallucination

9c. Task accessibility-guardian(PR content) - WCAG 2.1 AA compliance validation - Color contrast checking - Keyboard navigation validation - Screen reader support - Ensure distinctive design remains accessible
