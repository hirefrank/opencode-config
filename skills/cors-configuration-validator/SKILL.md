...
description: Automatically validates Cloudflare Workers CORS configuration, ensuring proper headers, OPTIONS handling, and origin validation for cross-origin requests. Activates when creating API responses, handling OPTIONS, or setting CORS headers.
triggers:
  [
    "Response creation",
    "API endpoints",
...
function getCorsHeaders(origin: string, env: Env) {
  ...
  return {
    ...
    "Access-Control-Allow-Methods":
      env.ALLOWED_METHODS || "GET, POST, PUT, DELETE",
    "Access-Control-Allow-Headers":
      env.ALLOWED_HEADERS || "Content-Type, Authorization",
    "Access-Control-Max-Age": "86400",
  };
}
